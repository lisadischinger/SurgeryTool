...
# 0x00-0x0F reserved for user-defined commands; LMD is to denote that Lisa M Dischinger made these commands*/

LMD_COMMAND = 0x01             # Trigger for when I am sending ot a command
LMD_IMU_ON = 0x02               # turn the IMU on
LMD_IMU_READ = 0x03            # Read the IMU
LMD_IMU_READ_REPLY = 0x04
LMD_IMU_OFF = 0x05              # turn the IMU off
...
class Board(object):
    ...
    def __init__(self, port, layout=None, baudrate=57600, name=None, timeout=None):
        ...
        # Setup handler for response data.                                            #LMD added from circuit Playground
        # Note that the data length (1) appears to be unused for these sysex
        # responses.
        print('updating the command handlers ', LMD_COMMAND)
        self._command_handlers.update({LMD_COMMAND: [self._response_handler, 1]})     #self._command_handler.command_dispatch.update({CP_COMMAND: [self._response_handler, 1]})
        #setup configured callbacks to null
        self._imu_callback = None                                       # LMD added
    ...
    # LMD added from circuit Playground to parse the different bytes to be sent over sysex
    ...
    def read_imu(self, callback):                                         # LMD added
        """Function created by LMD; this will read an IMU; might need to modify when there are multiple IMUs
        this will request an IMU reading. The result will be returned by calling the provided callback function and
        passing it the x, y, and z acceleration"""
        self._imu_callback = callback
        self.send_sysex(LMD_COMMAND, [LMD_IMU_READ])                         #self._command_handler.send_sysex(LMD_COMMAND, [LMD_IMU_READ])
        print("theoreticly 'sent_sysex'")
